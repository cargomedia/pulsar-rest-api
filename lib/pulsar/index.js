...
module.exports = (function() {
  ...
  Pulsar.prototype._validateTaskVariables = function(variables) {
    ...
    _.each(variables, function(value, key) {
      ...
      //regex below is (single word)|(words in quotes)
      if (!_.isString(value) || !/^(?:[^\s'"]+)|(?:(['"])[^'"]+\1)$/.test(value)) {
        errors.push('taskVariables.key:[' + key + '] contains illegal value. Value must be a single string or if it contains any whitespace, it must be quoted. Trailing spaces are illegal too.');
      }
    });
    if (errors.length) {
