  ...
  PulsarREST.prototype.installHandlers = function(server) {
    ...
    server.post('/:app/:env', function createTask(req, res, next) {
      ...
      self.pulsar.createTask(params.app, params.env, params.action, params.taskVariables, function(err, task) {
        if (err) {
          log.error('Task create failed', {params: params}, err);
          return next(err);
        }
        log.debug('Task create success', {params: params, task: task});
        var response = {
          id: task.id,
    ...
    server.get('/task/:id', function getTaskById(req, res, next) {
      ...
      self.pulsar.getTask(req.params.id, function(err, task) {
        if (err) {
          log.error('Task find failed', {id: req.params.id}, err);
          return next(err);
        }
        log.debug('Task find success', {id: req.params.id, task: task});
    ...
    server.post('/task/:id/kill', function killTaskById(req, res, next) {
      ...
      self.pulsar.getTask(req.params.id, function(err, task) {
        if (err) {
          log.error('Task kill failed', {id: req.params.id}, err);
          return next(err);
        }
        task.kill();
        res.send(200);
    ...
    server.get('/:app/:env/tasks', function getAvailableCapTasks(req, res, next) {
      self.pulsar.getAvailableCapTasks(req.params.app, req.params.env, function(err, tasks) {
        if (err) {
