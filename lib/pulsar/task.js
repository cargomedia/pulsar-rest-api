...
module.exports = (function() {
  ...
  PulsarTask.prototype.execute = function() {
    log.info('task', this.id, 'started execution');
    ...
    this._taskProcess.on('close', function(code, signal) {
      ...
      log.info('task', self.id, 'closed with status:', self.getStatus());
      self.onUpdate();
      self.onClose();
    });
  };
  ...
  PulsarTask.prototype.kill = function() {
    log.info('kill of task', this.id, 'started');
  ...
  PulsarTask.prototype._onKillError = function(err) {
    if (err.code !== 'ESRCH') {
      log.error('task', this.id, 'can\'t be killed');
