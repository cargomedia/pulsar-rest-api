...
module.exports = (function() {
  ...
  var PulsarDB = function(config) {
    ...
    this.connect(config.host || 'localhost', config.port || 27017, config.db || 'pulsar', config.collection || 'tasks');
  };
  ...
  PulsarDB.prototype.saveTask = function(task, callback) {
    this.collection.insert({_id: task.id, data: task.getData(), args: task.getArgs()}, function(err, focs) {
      callback(task);
    });
  };
