...
module.exports = (function() {
  ...
  function Server(config) {
    function init(err, db) {
      ...
      logger.createLog(configHash);
      var pulsar = new Pulsar(db, configHash.pulsar);
      var auth = configHash.auth ? new Auth(configHash.auth) : null;
      ...
      log.info('Pulsar API is ready and listening on port ' + configHash.port);
      log.debug('With config', configHash);
    }
