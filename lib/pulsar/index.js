...
module.exports = (function() {
  ...
  Pulsar.prototype.getAvailableCapTasks = function(app, env, callback) {
    var args = {
      app: app, env: env, capistranoOptions: ['--tasks']
    };
    var task = this._createTask(args);
    task.on('close', function() {
      if (task.status != PulsarTask.STATUS.FINISHED) {
        return callback(new Error('Collecting of available cap tasks finished abnormally.\n' + task.output));
      }
      var regex = /cap\s([^\s]+)\s+#\s+([^\s].+)\n/g;
      var match;
      var tasks = {};
      while (null !== (match = regex.exec(task.output))) {
