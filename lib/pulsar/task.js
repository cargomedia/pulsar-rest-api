...
module.exports = (function() {
  ...
  PulsarTask.prototype.execute = function() {
    log.debug('Task {id: ' + this.id + '} started');
    ...
    this._taskProcess.on('close', function(code, signal) {
      ...
      log.debug('Task {id: ' + self.id + '} stopped with status:' + self.getStatus());
      self.onUpdate();
      self.onClose();
    });
  };
  ...
  PulsarTask.prototype.kill = function() {
    log.debug('Task {id: ' + this.id + '} kill started');
  ...
  PulsarTask.prototype._onKillError = function(err) {
    if (err.code !== 'ESRCH') {
      log.error('Task {id: ' + this.id + '} can\'t be killed');
      this.status.set(PulsarStatus.FAILED);
      this.output = 'Warning: task wasn\'t killed ' + this.output;
      this.onUpdate();
    }
  };
