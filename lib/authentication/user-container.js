...
UserContainer.prototype.saveUser = function(userData, token) {
  ...
  this._userList[cookie] = {token: token, cookie: cookie, lastUsage: Date.now(), name: userData.login};
  return this._userList[cookie];
};

UserContainer.prototype.hasUserByCookie = function(cookie) {
  var user = this._userList[cookie];
  if (user) {
...
UserContainer.prototype.hasUserByToken = function(token) {
  var user = _.find(this._userList, function(user) {
