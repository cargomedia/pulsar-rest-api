...
module.exports = (function() {
  ...
  Pulsar.ILLEGAL_TASKS = ['invoke', 'shell'];
  ...
  Pulsar.prototype.createJob = function(app, env, task, taskVariables, callback) {
    ...
    task = task.trim();
    if (_.contains(Pulsar.ILLEGAL_TASKS, task)) {
      throw new ValidationError('Illegal pulsar task');
    }
    var args = {
      app: app,
      env: env,
   ...
   */
  Pulsar.prototype.getJobList = function(callback, currentPage, pageSize) {
   ...
   */
  Pulsar.prototype.restartJob = function(job, callback) {
    if (_.contains([PulsarJob.STATUS.KILLED, PulsarJob.STATUS.FAILED], job.status)) {
      ...
      throw new PulsarError('You can restart only KILLED or FAILED jobs', 500);
    }
  };
  ...
  Pulsar.prototype._createJob = function(args, data) {
    ...
    if (this.config.branch) {
      job._args.pulsarOptions.push('--conf-branch ' + this.config.branch);
    }
    return job;
  };
