    ...
    this.db.saveTask(task, function(err) {
      ...
      task.on('change', function() {
        self.db.updateTask(task, function(err) {
          if (err) {
            var msg = 'WARNING! Not all task\'s output was saved successfully';
            log.error(msg, 'for task:', task.id, err);
            task.output = msg + ' due to an error:' + err + '\n' + task.output;
            task.setStatus(PulsarStatus.FAILED);
            task.emit('change');
          }
        });
      });
