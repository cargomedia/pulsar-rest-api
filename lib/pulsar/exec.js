...
module.exports = (function() {
  ...
  PulsarExec.filterParams = function(params) {
    params = _.pick(params, 'app', 'env', 'action', 'pulsarOptions', 'capistranoOptions');
    if (params.pulsarOptions) {
      //it is forbidden to set pulsar options by user
      params.pulsarOptions = null;
    }
    if (params.capistranoOptions) {
      //user is allowed to pass only '-s' cap options
      var errors = [];
      _.each(params.capistranoOptions, function(elem, index, list) {
        if (shellwords.split(elem).length > 1 || elem.indexOf('=') === -1) {
          errors.push(elem + ' is illegal capistrano option. The supported format is <key>=<value>. There must be no whitespace between <key>,\'=\' and <value>');
        }
        list[index] = '-s ' + elem;
      });
      if (errors.length) {
